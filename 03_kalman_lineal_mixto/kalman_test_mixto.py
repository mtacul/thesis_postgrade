...
b_body_i = functions_03.rotacion_v(q_est, bi_orbit, 1e-6)

si_orbit = [vx_sun_orbit[0],vy_sun_orbit[0],vz_sun_orbit[0]]
s_body_i = functions_03.rotacion_v(q_est, si_orbit, 0.036)
hh =0.01
...
for i in range(len(t)-1):
    ...
    b_body = functions_03.rotacion_v(q_est, b_orbit, sigma_b)

    vsun_orbit = [vx_sun_orbit[i+1],vy_sun_orbit[i+1],vz_sun_orbit[i+1]]
    s_body = functions_03.rotacion_v(q_est, vsun_orbit, sigma_ss)
    ...
    [q_posteriori, w_posteriori, P_k_pos,K_k] = functions_03.kalman_lineal(A_discrete, B_discrete,C_discrete, x_est, u_est, b_body, s_body, P_ki, sigma_b, sigma_ss, deltat,hh)
    # asasd
...
axes0[0].legend()
axes0[0].set_title('cuaterniones estimados por el filtro de kalman lineal discreto')
axes0[0].grid()
...
axes0[0].plot(t, w0_est, label='w0 kalman')
axes0[0].plot(t, w1_est, label='w1 kalman')
axes0[0].plot(t, w2_est, label='w2 kalman')
axes0[0].set_xlabel('Tiempo [s]')
axes0[0].set_ylabel('velocidad angular [rad/s]')
axes0[0].legend()
axes0[0].set_title('velocidades angulares estimados por el filtro de kalman lineal discreto')
axes0[0].grid()
# axes0[0].set_ylim(-1, 1)  # Ajusta los límites en el eje Y
