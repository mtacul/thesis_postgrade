...
b_body_i = functions_04.rotacion_v(q_est, bi_orbit, sigma_b)

si_orbit = [vx_sun_orbit[0],vy_sun_orbit[0],vz_sun_orbit[0]]
s_body_i = functions_04.rotacion_v(q_est, si_orbit, sigma_ss)
hh =0.01
...
for i in range(len(t)-1):
    print(t[i+1])
    u_est = np.array([1500,1500,1500])
    

    b_orbit = [Bx_orbit[i+1],By_orbit[i+1],Bz_orbit[i+1]]
    b_body = functions_04.rotacion_v(q_est, b_orbit, sigma_b)

    vsun_orbit = [vx_sun_orbit[i+1],vy_sun_orbit[i+1],vz_sun_orbit[i+1]]
    s_body = functions_04.rotacion_v(q_est, vsun_orbit, sigma_ss)
