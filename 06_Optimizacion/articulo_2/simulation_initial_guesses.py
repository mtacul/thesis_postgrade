...
def generate_initial_guesses(bounds, num_points_per_dim=3):
    """
...
results_acc = results[0:27]
results_time = results[27:54]
results_all = results[54:81]
...
for idx, ((x, f), guess) in enumerate(zip(results_time, initial_guesses)):
    ...
    print(f"  x óptimo: {x}")
    print(f"  Valor de función objetivo: {f}\n")
for idx, ((x, f), guess) in enumerate(zip(results_all, initial_guesses)):
    print(f"Optimización {idx+1}:")
    print(f"  Initial guess: {guess}")
    print(f"  x óptimo: {x}")
    print(f"  Valor de función objetivo: {f}\n")
...
# optimal_x = [x[0] for x, _ in results_all]
# optimal_y = [x[1] for x, _ in results_all]
# optimal_z = [x[2] for x, _ in results_all]
