...
opcion = int(input("Seleccione una opción (1: bad, 2: med, 3: good, 4: sin ruido): "))
...
for i in range(len(t)-1):
    ...
    b_body_est = functions_06_rw.rotacion_v(q_est, b_orbit,sigma_b)
    
    s_orbit = [vx_sun_orbit[i],vy_sun_orbit[i],vz_sun_orbit[i]]
    s_body_med = functions_06_rw.rotacion_v(q_real, s_orbit,sigma_ss)
    s_body_est = functions_06_rw.rotacion_v(q_est, s_orbit,sigma_ss)
    ...
    if opcion == 4:
        [q_posteriori, w_posteriori, P_k_pos,K_k, ws_posteriori] = functions_06_rw.kalman_lineal(A_discrete, B_discrete,C_discrete, x_est, u_est, b_body_med, b_body_est, s_body_med, s_body_est, P_ki, 0.012e-6, 0.05, deltat,hh,h_real,h_est)
    elif opcion == 1 or opcion == 2 or opcion == 3:
        [q_posteriori, w_posteriori, P_k_pos,K_k, ws_posteriori] = functions_06_rw.kalman_lineal(A_discrete, B_discrete,C_discrete, x_est, u_est, b_body_med, b_body_est, s_body_med, s_body_est, P_ki, sigma_b, sigma_ss, deltat,hh,h_real,h_est)
...
[MSE_cuat, MSE_omega]  = functions_06_rw.cuat_MSE_NL(q0_real, q1_real, q2_real, q3_real, w0_real, w1_real, w2_real, q0_est, q1_est, q2_est, q3_est, w0_est, w1_est, w2_est)   
[RPY_all_est,RPY_all_id,mse_roll,mse_pitch,mse_yaw] = functions_06_rw.RPY_MSE(t, q0_est, q1_est, q2_est, q3_est, q0_real, q1_real, q2_real, q3_real)   
