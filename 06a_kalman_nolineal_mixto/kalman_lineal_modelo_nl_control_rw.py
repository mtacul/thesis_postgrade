...
opcion = int(
    input("Seleccione una opción (1: bad, 2: med, 3: good, 4: sin ruido): "))
...
x_real = np.hstack(
    (np.transpose(q_real[:3]), np.transpose(w_gyros), np.transpose(ws_real)))
h_real = np.array([x_real[6]/I_s0_x-x_real[3], x_real[7] /
                  I_s1_y-x_real[4], x_real[8]/I_s2_z-x_real[5]])
...
data = [[8.87572,	0,	0,	-25.8521,	0,	0,	3.09236,	0,	0],
        [0,	156.945,	-0.00176661,	0,	4.69022,	-0.0812786,	0,	0.0394282,	0.0315633],
        [0,	0.0199751,	16.532,	0, 0.142849,	1.46647,	0,	-0.0168054,	-0.0132888]
        ]

K = np.array(data)

diagonal_values = np.array(
    [0.5**2, 0.5**2, 0.5**2, 0.1**2, 0.1**2, 0.1**2, 0.01**2, 0.01**2, 0.01**2])
...
for i in range(len(t)-1):
    ...
    [xx_new_d, qq3_new_d] = functions_06_rw.mod_nolineal(
        x_real, u_est, deltat, hh, I_x, I_y, I_z, w0_O, I_s0_x, I_s1_y, I_s2_z, J_x, J_y, J_z)
...
[MSE_cuat, MSE_omega] = functions_06_rw.cuat_MSE_NL(
    q0_real, q1_real, q2_real, q3_real, w0_real, w1_real, w2_real, q0_est, q1_est, q2_est, q3_est, w0_est, w1_est, w2_est)
[RPY_all_est, RPY_all_id, mse_roll, mse_pitch, mse_yaw] = functions_06_rw.RPY_MSE(
    t, q0_est, q1_est, q2_est, q3_est, q0_real, q1_real, q2_real, q3_real)
...
axes0[1].legend()
axes0[1].set_title(
    'cuaterniones estimados por el filtro de kalman lineal discreto')
axes0[1].grid()
plt.tight_layout()
plt.show()
...
axes0[0].legend()
axes0[0].set_title(
    'velocidades angulares obtenidos por el modelo de control lineal discreto')
axes0[0].grid()
# axes0[0].set_ylim(-1, 1)  # Ajusta los límites en el eje Y
...
axes0[1].legend()
axes0[1].set_title(
    'velocidades angulares estimados por el filtro de kalman lineal discreto')
axes0[1].grid()
plt.tight_layout()
plt.show()
...
axes0[1].legend()
axes0[1].set_title(
    'velocidades angulares estimados en las tres ruedas de reaccion')
axes0[1].grid()
plt.tight_layout()
plt.show()
...
plt.legend()
plt.title(
    'MSE de cada velocidad angular entre lineal discreto y kalman lineal discreto')
# plt.xlim(20000,100000)
# plt.ylim(-0.005,0.005)
plt.grid()
plt.show()
...
axes0[0].legend()
axes0[0].set_title(
    'Angulos de Euler obtenidos por el modelo de control lineal discreto')
axes0[0].grid()
# axes0[0].set_ylim(-1, 1)  # Ajusta los límites en el eje Y
...
axes0[1].legend()
axes0[1].set_title(
    'Angulos de Euler estimados por el filtro de kalman lineal discreto')
axes0[1].grid()
plt.tight_layout()
plt.show()
...
nombre_archivo = f"_sen{opcion}_act{opcion_tau}_RW_nl.csv"
