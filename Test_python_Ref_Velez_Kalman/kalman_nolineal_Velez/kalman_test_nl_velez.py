...
w_gyros = functions_05.simulate_gyros_reading(w_body, 0,0)
x_real = np.hstack((np.transpose(q_real[:3]), np.transpose(w_gyros)))

bi_orbit = [Bx_orbit[0],By_orbit[0],Bz_orbit[0]]
b_body_i = functions_05.rotacion_v(q_real, bi_orbit, sigma_b)

si_orbit = [vx_sun_orbit[0],vy_sun_orbit[0],vz_sun_orbit[0]]
s_body_i = functions_05.rotacion_v(q_real, si_orbit, sigma_ss)
hh =0.01
...
for i in range(len(t)-1):
    ...
    b_body = functions_05.rotacion_v(q_real, b_orbit, sigma_b)

    vsun_orbit = [vx_sun_orbit[i+1],vy_sun_orbit[i+1],vz_sun_orbit[i+1]]
    s_body = functions_05.rotacion_v(q_real, vsun_orbit, sigma_ss)

    [A,B,C,A_discrete,B_discrete,C_discrete] = functions_05.A_B(I_x, I_y, I_z, w0_O, q_est[0],q_est[1],q_est[2],w_est[0], w_est[1], w_est[2], deltat, hh, b_body, b_orbit)
    [q_posteriori, w_posteriori, P_k_pos,K_k] = functions_05.kalman_lineal(A_discrete, B_discrete,C_discrete, x_real, u_est, b_body, b_orbit, P_ki, sigma_b, sigma_ss, deltat,w_gyros,hh)
    # asasd
    ...
    [xx_new_d, qq3_new_d] = functions_05.mod_nolineal(
